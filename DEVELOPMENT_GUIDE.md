...
## 프로젝트 개요
- 프로젝트명: [이름]
- 기술 스택: [Next.js 16 / PHP 8 / etc.]
- DB: [Supabase / MySQL / etc.]
- 배포: [Railway / Netlify / etc.]

## 파일 구조 원칙
- 공통 로직은 1곳에만 정의 (Single Source of Truth)
...
- [ ] `src/lib/constants.ts` 수정 (1곳만)
- [ ] `npm run build` — 빌드 에러로 누락 감지
- [ ] grep으로 이전 값 잔존 확인

```
Claude Code 프롬프트:
"constants.ts에서 [항목]을 변경했어.
이전 값이 다른 파일에 남아있는지 검색해줘.
코드 수정은 하지 말고 검색만 해줘."
```
...
Claude Code 프롬프트:
"[API 엔드포인트]를 변경했어.
이 API를 호출하는 프론트엔드 파일을 모두 찾아줘.
코드 수정은 하지 말고 목록만 보여줘."
```
...
Claude Code 프롬프트:
"DB에 [테이블/컬럼]을 추가했어.
이 테이블을 사용하는 API와 페이지를 모두 찾아줘.
영향 범위를 분석해줘."
```

### 컴포넌트 변경 시

- [ ] props 타입 변경 시 → 사용하는 모든 곳 수정
- [ ] 컴포넌트 이름 변경 시 → import 경로 전부 수정

```
Claude Code 프롬프트:
"[컴포넌트명]의 props를 변경했어.
이 컴포넌트를 사용하는 파일을 모두 찾아줘."
```
