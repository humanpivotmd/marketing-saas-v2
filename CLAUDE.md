...
## 🔄 Co-update Map (전방 영향 검증 — 학습형 시스템)

> **목적**: 새 기능 추가 시 "이것도 같이 해야 하지 않나?" 자동 체크 + 시간이 지날수록 시스템이 똑똑해짐.

### 두 파일로 분리됨
- **[`.md/co-update/patterns.md`](.md/co-update/patterns.md)** — 일반 규칙 12개 (안정적, 천천히 업데이트)
- **[`.md/co-update/cases.md`](.md/co-update/cases.md)** — 구체 사례 누적 (학습 데이터, 빠르게 증가)

### 사용 방법
1. 사용자가 새 기능 요청 → `@designer` 호출
2. designer가 `patterns.md`에서 매칭되는 패턴 검색 (여러 패턴 동시 매칭 가능)
3. designer가 `cases.md`에서 유사 사례 N건 검색해서 보고서에 표시
4. 매칭된 항목 전부를 spec 보고서에 포함 → 사용자 승인 → 진행

### 학습 루프
1. 사용자가 "이거 또 빠졌어" → 자동 또는 수동으로 `cases.md`에 기록:
   ```bash
   node "F:/marketing -app/agent-dev-kit/scripts/case-logger.mjs" \
     "<사례 설명>" --pattern=N --commit=<sha> --category=<태그>
   ```
2. 같은 카테고리에 3건 이상 누적되면 패턴 보강 후보 자동 제안:
   ```bash
   node "F:/marketing -app/agent-dev-kit/scripts/pattern-extractor.mjs"
   ```
3. 사용자가 검토 → 승인 → `patterns.md` 업데이트
4. 다음 비슷한 케이스에서 designer가 자동 매칭

자세한 패턴 목록은 `patterns.md` 참조. 12개 패턴이 정의되어 있고, 각 패턴마다 관련 사례 번호 링크.
